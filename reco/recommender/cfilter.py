...
class CFRecommender:
    ...
    def fit(self, data, formatizer = None):
        ...
        for i in range(0,len(data)):
            item=str(data.iloc[i][self.formatizer['item']])
            user=str(data.iloc[i][self.formatizer['user']])
            if user!=past_user:
                ratings[user]={}
            count=count+1

            ratings[user][item]=float(data.iloc[i][self.formatizer['value']])
            past_user=user
    ...
    def predict(self, X, formatizer = None):
        ...
        for i in range(len(X)):
            ans = self.predict_single(X.iloc[i][self.formatizer['user']], X.iloc[i][self.formatizer['item']])
            out.append(ans)
